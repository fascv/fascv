...
def _gate_report(
    ...
    *,
    ...
    holdout_min_trades: float,
    min_oos_return: float,
    min_oos_trades: float,
    max_holdout_cost_bps: float,
    ...
    holdout_trades = float(holdout.get("trades", 0.0) or 0.0)
    holdout_cost = float(holdout.get("avg_realized_cost_bps", 0.0) or 0.0)
    ...
    checks = {
        ...
        "holdout_trades": holdout_trades >= float(holdout_min_trades),
        "holdout_cost": holdout_cost <= float(max_holdout_cost_bps),
    ...
    metrics = {
        ...
        "holdout_avg_realized_cost_bps": holdout_cost,
...
def main() -> None:
    p = argparse.ArgumentParser(
        description=(
            "Governed professional optimizer: multi-strategy walk-forward rounds with strict "
            "acceptance gates for holdout and OOS quality."
        )
    )
    ...
    p.add_argument("--holdout-min-trades", type=float, default=3.0)
    p.add_argument("--holdout-max-realized-cost-bps", type=float, default=40.0)
    p.add_argument("--oos-min-return", type=float, default=0.0)
    p.add_argument("--oos-min-trades", type=float, default=30.0)

    p.add_argument("--out-yaml-final", default="configs/sim_optimized_professional_governed.yaml")
    p.add_argument("--out-json-final", default="reports/sim_opt_report_professional_governed.json")
    p.add_argument("--governance-json", default="reports/professional_governance_summary.json")
    p.add_argument("--governance-log", default="reports/professional_governance_runs.jsonl")
    p.add_argument(
        "--round-prefix",
        default="",
        help="Optional prefix for per-round artifact tags (prevents collisions across multi-asset runs).",
    )
    args = p.parse_args()
    ...
    summary: Dict[str, Any] = {
        ...
        "gates": {
            ...
            "holdout_min_trades": float(args.holdout_min_trades),
            "holdout_max_realized_cost_bps": float(args.holdout_max_realized_cost_bps),
    ...
    round_prefix = str(args.round_prefix or "").strip()
    for i, (trials, seed) in enumerate(rounds, start=1):
        base_tag = f"r{i:02d}_t{trials}_s{seed}"
        tag = f"{round_prefix}_{base_tag}" if round_prefix else base_tag
        out_yaml = root / f"configs/sim_optimized_professional_{tag}.yaml"
        out_json = root / f"reports/sim_opt_report_professional_{tag}.json"
        profile_json = root / f"reports/data_profile_professional_{tag}.json"
        ...
        cmd = [
            ...
            "--optimizer-script",
            "scripts/optimize_sim_professional.py",
            "--optimizer-arg=--families",
            f"--optimizer-arg={str(args.families)}",
            "--out-yaml",
            str(out_yaml),
            "--out-json",
            str(out_json),
        ...
        try:
            ...
            passed, gate = _gate_report(
                ...
                max_holdout_cost_bps=float(args.holdout_max_realized_cost_bps),
            )
            round_info["status"] = "ok"
            round_info["gate"] = gate
            round_info["passed"] = bool(passed)
            round_info["best_family"] = str(report.get("best", {}).get("family", ""))
        ...
        print(
            json.dumps(
                {
                    "round": tag,
                    "passed": round_info.get("passed"),
                    "status": round_info.get("status"),
                    "best_family": round_info.get("best_family"),
                },
                ensure_ascii=True,
            ),
            flush=True,
    ...
    if accepted_round is not None:
        ...
        summary["accepted"] = {
            ...
            "best_family": accepted_round.get("best_family"),
